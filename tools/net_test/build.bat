...
::   1  the tests ran and FAILED, or would not compile
...
cl.exe /nologo /std:c++20 /EHsc /W3 /DWIIXL_HOST_TEST=1 /I"%ROOT%\include" ^
    /Fe:net_test.exe /Fo:net_test.obj main.cpp
if %ERRORLEVEL% NEQ 0 (
    echo [net_test] COMPILE FAILED
    popd
    exit /b 1
)
"%HERE%net_test.exe"
...
:: Found when hook_test started crashing and the build stayed green; every
:: gate in this tree carries the same guard now.
::
:: Any non-zero result becomes 1, so it cannot be mistaken for either
:: success or the exit-2 "toolchain missing" case.
if not "%RC%"=="0" (
    echo [net_test] FAILED or CRASHED - process exit code %RC%.
    exit /b 1
)
exit /b 0
