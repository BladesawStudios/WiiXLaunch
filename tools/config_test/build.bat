...
if not exist "%VSWHERE%" (
    echo [config_test] vswhere.exe not found - is Visual Studio installed?
    exit /b 2
)
...
if "%VSINSTALL%"=="" (
    echo [config_test] No Visual Studio with the C++ tools found.
    exit /b 2
)
...
:build
pushd "%HERE%"
cl.exe /nologo /std:c++17 /EHsc /I. /I"%HERE%..\..\include" /Fe:config_test.exe /Fo:config_test.obj main.cpp
if %ERRORLEVEL% NEQ 0 (
    echo [config_test] COMPILE FAILED
    popd
    exit /b 1
)
echo.
"%HERE%config_test.exe"
set RC=%ERRORLEVEL%
popd
...
:: Found when config_test started crashing and the build stayed green.
::
:: Any non-zero result becomes 1, so it cannot be mistaken for either
:: success or the exit-2 "toolchain missing" case.
if not "%RC%"=="0" (
    echo [config_test] FAILED or CRASHED - process exit code %RC%.
    exit /b 1
)
exit /b 0
