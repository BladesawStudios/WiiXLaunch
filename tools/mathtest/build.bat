...
if not exist "%VSWHERE%" (
    echo [mathtest] vswhere.exe not found - is Visual Studio installed?
    exit /b 2
)
...
if "%VSINSTALL%"=="" (
    echo [mathtest] No Visual Studio with the C++ tools found.
    exit /b 2
)
...
:build
pushd "%HERE%"
cl.exe /nologo /std:c++17 /EHsc /I. /Fe:mathtest.exe /Fo:mathtest.obj main.cpp
if %ERRORLEVEL% NEQ 0 (
    echo [mathtest] COMPILE FAILED
    popd
    exit /b 1
)
echo.
...
:: Found when mathtest started crashing and the build stayed green.
::
:: Any non-zero result becomes 1, so it cannot be mistaken for either
:: success or the exit-2 "toolchain missing" case.
if not "%RC%"=="0" (
    echo [mathtest] FAILED or CRASHED - process exit code %RC%.
    exit /b 1
)
exit /b 0
