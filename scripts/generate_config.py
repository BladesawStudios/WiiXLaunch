...
"""
    ...
    id_sw = ident.get("switch", {}) or {}
    id_wu = ident.get("wiiu", {}) or {}
    sw_off = _num(id_sw.get("offset"))
    sw_len = _num(id_sw.get("length"))
    wu_addr = _num(id_wu.get("address"))
    ...
    def _known(platform):
        ...
        for e in ident.get("known", []) or []:
            if (e.get("platform") or "").lower() != platform:
                continue
            rows.append('        { 0x%08X, "%s" },'
                        % (_num(e.get("fingerprint")), e.get("name", "?")))
        rows.append("        { 0, nullptr },")
        return "\n".join(rows)

    known_sw = _known("switch")
    known_wu = _known("wiiu")
...
namespace WiiXLaunch::Host {{
    ...
    constexpr uint32_t IdentitySwitchOffset = 0x{sw_off:08X};
    constexpr uint32_t IdentitySwitchLength = {sw_len};
    constexpr uint32_t IdentityWiiuAddress  = 0x{wu_addr:08X};
    constexpr uint32_t IdentityWiiuLength   = {wu_len};
