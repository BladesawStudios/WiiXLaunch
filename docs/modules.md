...
* **A global that no C++ code references needs `__attribute__((used))`.** If a symbol is written by one tool and read by another - a shim-table offset that `scripts/deploy.py` patches and a `src/cemu/*.asm` table reads through, and later anything the module loader fills in - then no translation unit odr-uses it, and GCC never emits an `inline` variable nobody odr-uses. The symbol is then absent from the ELF, deploy.py has nothing to patch, and the table ships in the codecave with every call through it reading a null pointer. Nothing fails at build time and nothing says anything at boot. Declare these as `__attribute__((section(".data"), used)) inline uint32_t g_Whatever = 0;` - see `include/wiixlaunch/mem.hpp`, whose shims shipped unreachable for exactly this reason. deploy.py now refuses to build a pack whose `WIIXL_OFFSET_SYMBOL` did not resolve.
